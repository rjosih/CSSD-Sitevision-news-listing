...
const App = ({ dataObj, quantity }) => {
  const [news] = React.useState(dataObj);
    ...
    <div className="env-cardholder-grid">
      {news.length > 0 ? news.map((article, index) => {
        return (
          <>
            {index < quantity ?  
              <article key={article.NODE_ID} className="env-card env-block env-shadow">
